...
exports.sanitizeKey = (input) => {
  ...
  // 5. Truncate to avoid extremely long keys (S3 limit is 1024, but we keep it reasonable)
  sanitized = sanitized.substring(0, 180);

  // 6. Final safety: if the result is empty (e.g. input was only emojis), provide a fallback
